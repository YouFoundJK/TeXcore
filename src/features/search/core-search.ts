...
export class ActiveNoteSearchCore {
    ...
    setScope() {
        this.scope.register([this.plugin.settings.modifierToJump], "Enter", () => {
            ...
            openFileAndSelectPosition(this.app, file, item.$pos, ...LEAF_OPTION_TO_ARGS[this.plugin.settings.suggestLeafOption]);
        ...
        if (this.plugin.settings.showModifierInstruction) {
            this.parent.setInstructions([
                ...
                { command: `${getModifierNameInPlatform(this.plugin.settings.modifierToJump)} + ↵`, purpose: "to jump" },
            ]);
        }
    }
    ...
    async getSuggestions(query: string): Promise<EquationBlock[]> {
        ...
        const callback = (this.plugin.settings.searchMethod == "Fuzzy" ? prepareFuzzySearch : prepareSimpleSearch)(query);
        const results: ScoredEquationBlock[] = [];

        for (const block of blocks) {
            const result = callback(block.$mathText);
            if (result) { results.push({ match: result, block }); }
        }
