 ...
 */
export function getMathLink(plugin: LatexReferencer, targetLink: string, sourcePath: string): string {
	...
	for (const provider of plugin.internalProviders) {
		const provided = provider.provide({ path, subpath }, targetFile, subpathResult);
		if (provided) {
			return provided;
		}
	}
